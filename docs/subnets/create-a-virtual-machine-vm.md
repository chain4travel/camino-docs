...
One of the core features of Camino is the ability to create new, custom blockchains, which are defined by [Virtual Machines (VMs)](../about/#virtual-machines)

In this tutorial, we’ll create a very simple VM. The blockchain defined by the VM is a [TimestampVM](https://github.com/chain4travel/camino-timestampvm/). Each block in the blockchain contains the timestamp when it was created along with a 32-byte piece of data (payload). Each block’s timestamp is after its parent’s timestamp.
...
- The code in latest version/branch can be different than ones presented in this page.
...
To reach consensus on linear blockchains (as opposed to DAG blockchains), Camino uses the Snowman consensus engine. In order to be compatible with Snowman, a VM must implement the `block.ChainVM` interface, which can be accessible from [CaminoGo repository](https://github.com/chain4travel/caminogo/blob/chain4travel/snow/engine/snowman/block/vm.go).
...
You can see the full file from [here.](https://github.com/chain4travel/caminogo/blob/chain4travel/snow/engine/common/vm.go)
...
Let’s look at this interface and its methods. You can see the full file from [here.](https://github.com/chain4travel/caminogo/blob/chain4travel/snow/consensus/snowman/block.go)
...
This interface is the superset of every decidable object, such as transactions, blocks and vertices. You can see the full file from [here.](https://github.com/chain4travel/caminogo/blob/chain4travel/snow/choices/decidable.go)
...
`rpcchainvm` is a special VM that wraps a `block.ChainVM` and allows the wrapped blockchain to run in its own process separate from Camino-Node. `rpcchainvm` has two important parts: a server and a client. The [server](https://github.com/chain4travel/caminogo/blob/chain4travel/vms/rpcchainvm/vm_server.go) runs the underlying `block.ChainVM` in its own process and allows the underlying VM's methods to be called via gRPC. The [client](https://github.com/chain4travel/caminogo/blob/chain4travel/vms/rpcchainvm/vm_client.go) runs as part of CaminoGo and makes gRPC calls to the corresponding server in order to update or query the state of the blockchain.
...
Let’s write our VM, which implements `block.ChainVM` and whose blocks implement `snowman.Block`. You can also follow the code in the [TimestampVM repository](https://github.com/chain4travel/camino-timestampvm/).
...
A VM may have a static API, which allows clients to call methods that do not query or update the state of a particular blockchain, but rather apply to the VM as a whole. This is analogous to static methods in computer programming. CaminoGo uses [Gorilla’s RPC library](https://www.gorillatoolkit.org/pkg/rpc) to implement HTTP APIs.
...
This VM has a [build script](https://github.com/chain4travel/camino-timestampvm/blob/camino-refactor/scripts/build.sh) that builds an executable of this VM (when invoked, it runs the `main` method from above.)
...
[build directory](../apps/nodes/camino-node-config-flags.md#build-directory).
