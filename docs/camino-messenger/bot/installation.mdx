...
<figure>
  <img class="zoom" src="/img/messenger/tags.png" alt="This image displays the Camino Messenger bot repo tags" />
</figure>
...
# Checkout a specific tag: replace <tag> with the desired version (e.g., v10.1.0)
git checkout <tag>
...
To pull a specific version, replace `latest` with the desired version tag (e.g., `v10.1.0`).
...
git clone -b c4t https://github.com/chain4travel/camino-messenger-bot.git
cd camino-messenger-bot
#checkout certain tag. replace with the tag (eg: v10.1.0)
git checkout
git submodule update --init
```
