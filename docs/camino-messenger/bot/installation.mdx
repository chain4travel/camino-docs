...
When accessing our GitHub repository, note that the default branch is dev.
Always switch to the latest version using the available tags or the latest reference.
[Camino Messenger Bot Github repository](https://github.com/chain4travel/camino-messenger-bot)
...
- **Cancellation is not yet implemented:** The bot currently does not support cancellation
  as specified in the V11 protocol. This functionality will be included in a future release.
...
- **Listening to events:** The bot is not yet persisting subscriptions to blockchain
  events. This means that if the bot is restarted after a mint operation, but before
  the buy operation has finished, the supplier bot will not be notified of the successful
  transfer of the NFT.
...
- **Crash on Empty Message:** The bot crashes if it receives an empty message, which
  ...
  types will be implemented using the proto validate library.
