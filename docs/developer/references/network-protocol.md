...
Camino network defines the core communication format between Camino nodes. It uses the [primitive serialization](serialization-primitives.md) format for payload packing.
...
`Version` ensures that the nodes we are connected to are running compatible versions of Camino, and at least loosely agree on the current time.
...
Sending a `Version` message with the time `November 16th, 2008 at 12:00am (UTC)` and the version `camino/0.0.1`
