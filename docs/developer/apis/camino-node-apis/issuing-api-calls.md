...
- `http-port` is the port the node listens on for HTTP calls. This is specified by [command-line argument](/nodes/camino-node-config-flags.md#http-server) `http-port` (default value `9650`).
