...
_Note: Ethereum has its own notion of `networkID` and `chainID`. These have no relationship to Camino’s view of networkID and chainID and are purely internal to the_ [_C-Chain_](../../learn/platform-overview/README.md#contract-chain-c-chain)_. On Mainnet, the C-Chain uses `1000` and `500` for these values. On the Columbus Testnet, it uses `1002` and `502` for these values. `networkID` and `chainID` can also be obtained using the `net_version` and `eth_chainId` methods._

## Deploying a Smart Contract

[Deploy a Smart Contract on Camino using Remix and MetaMask](../tutorials/smart-contracts/deploy-a-smart-contract-on-Camino-using-remix-and-metamask.md)
