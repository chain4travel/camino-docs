...
The [X-Chain](../../learn/platform-overview/README.md#exchange-chain-x-chain), Camino’s native platform for creating and trading assets, is an instance of the Avalanche Virtual Machine (AVM). This API allows clients to create and trade assets on the X-Chain and other instances of the AVM.
...
"genesisData" :
    {
        "assetAlias1": {               // Each object defines an asset
            ...
            "symbol":"TOUR",           // Symbol is between 0 and 4 characters
            "initialState": {
        ...
        "assetAliasCanBeAnythingUnique": { // Asset alias can be used in place of assetID in calls
            ...
            "symbol": "TOUR",              // symbols need not be unique
            "initialState": {
            ...
            "asset1": {
                ...
                "initialState": {
                    "fixedCap" : [
                        {
                            "amount":100000,
                            "address": "columbus13ery2kvdrkd2nkquvs892gl8hg7mq4a6ufnrn6"
                        },
                        {
                            "amount":100000,
                            "address": "columbus1rvks3vpe4cm9yc0rrk8d5855nd6yxxutfc2h2r"
                        },
                        {
                            "amount":50000,
                            "address": "columbus1ntj922dj4crc4pre4e0xt3dyj0t5rsw9uw0tus"
                        },
                        {
                            "amount":50000,
                            "address": "columbus1yk0xzmqyyaxn26sqceuky2tc2fh2q327vcwvda"
                        }
                    ]
                }
            },
            "asset2": {
                ...
                "initialState": {
                    "variableCap" : [
                        {
                            "minters": [
                                "columbus1kcfg6avc94ct3qh2mtdg47thsk8nrflnrgwjqr",
                                "columbus14e2s22wxvf3c7309txxpqs0qe9tjwwtk0dme8e"
                            ],
                            "threshold":1
                        },
                        {
                            "minters": [
                                "columbus1y8pveyn82gjyqr7kqzp72pqym6xlch9gt5grck",
                                "columbus1c5cmm0gem70rd8dcnpel63apzfnfxye9kd4wwe",
                                "columbus12euam2lwtwa8apvfdl700ckhg86euag2hlhmyw"
                            ],
                            "threshold":2
                        }
                    ]
...
curl -X POST --data '{
    ...
    "params" :{
        ...
        "minterSets":[
            {
                "minters":[
                    "X-columbus14q0p6y4yzweuugz9p080kapajwvac3ur755n7d"
                ],
                "threshold": 1
            },
            {
                "minters": [
                    "X-columbus1fzyldr3mwn6lj7y46edhua6vr5ayx0ruuhezpv",
                    "X-columbus1x5mrgxj0emysnnzyszamqxhq95t2kwcp9n3fy3",
                    "X-columbus13zmrjvj75h3578rn3sfth8p64t2ll4gm4tv2rp"
                ],
                "threshold": 2
            }
        ],
...
[P-Chain's `platform.import`](p-chain.md#platformimport) to complete the transfer.
...
* `assetID` is the asset id of the asset which is sent. Use `CAM` for CAM exports.
* `from` are the addresses that you want to use for this operation. If omitted, uses any of your addresses as needed.
* `changeAddr` is the address any change will be sent to. If omitted, change is sent to one of the addresses controlled by the user.
...
curl -X POST --data '{
    ...
    "params" :{
        "username":"myUsername",
        "password":"myPassword",
        "address":"X-columbus1jggdngzc9l87rgurmfu0z0n0v4mxlqta0h3k6e"
    }
}' -H 'content-type:application/json;' 127.0.0.1:9650/ext/bc/X
```
...
curl -X POST --data '{
    ...
    "params" :{
        "address":"X-columbus1c79e0dd0susp7dc8udq34jgk2yvve7hapvdyht"
    }
}' -H 'content-type:application/json;' 127.0.0.1:9650/ext/bc/X
```
...
* `assetID` is the id of the asset for which the information is requested. If empty, the native asset is returned.
                ...
                "outputs": [
                    {
                        ...
                        "output": {
                            "addresses": [
                                "X-columbus126rd3w35xwkmj8670zvf7y5r8k36qa9z9803wm"
                            ],
                            "amount": 1530084210,
                            "locktime": 0,
                            "threshold": 1
...
* `networkID` is the ID of the network this transaction happened on. (Camino Mainnet is `1000`.)
* `blockchainID` is the ID of the blockchain this transaction happened on.
...
* `assetID`: The ID of the asset being transferred. (The Columbus CAM ID is `o8seyjX6WupqJ1CE8CeaozK13kqVgc4DFvdvc4crfacLFBauW`.)
...
curl -X POST --data '{
    ...
    "method" :"avm.getUTXOs",
    "params" :{
        "addresses":["X-columbus1x459sj0ssujguq723cljfty4jlae28evjzt7xz"],
        "limit":5,
        "endIndex": {
            "address": "X-columbus1x459sj0ssujguq723cljfty4jlae28evjzt7xz",
            "utxo": "kbUThAUfmBXUmRgTpgD6r3nLj7rJUGho6xyht5nouNNypH45j"
...
* `to` is the address the CAM is sent to. This must be the same as the `to` argument in the corresponding call to the P-Chain’s `exportAVAX` or C-Chain's `export`.
* `sourceChain` is the ID or alias of the chain the CAM is being imported from. To import funds from the C-Chain, use `"C"`.
...
* Sends `amount` units of asset with ID `assetID` to address `to`. `amount` is denominated in the smallest increment of the asset. For CAM this is 1 nCAM (one billionth of 1 CAM.)
...
func main() {
    ...
    addresses = append(addresses, " X-columbus....")
